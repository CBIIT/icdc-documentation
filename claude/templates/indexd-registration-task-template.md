...
- **No Developer field — by design.** IndexD registration is coordination with DCF (the external indexing team), not engineering; nothing is built. Leave `customfield_23650` empty — an empty Developer field is the *correct* state for this ticket type. The Assignee (typically the TPM) carries ownership. See SKILL.md §3.
- **Apply the `Data-Concierge` label.** IndexD registration is part of the Data Concierge service, so this ticket type carries the `Data-Concierge` label (confirmed on ICDC-4175). The paired Data Loading Task does *not* — that's engineering. See SKILL.md §3.
