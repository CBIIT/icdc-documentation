...
const pts = x => (typeof x.points === 'number' ? x.points : null), sum = a => a.reduce((s, x) => s + (pts(x) || 0), 0);
console.log('story points: committed', sum(d), '| delivered', sum(done), '| carried', sum(d) - sum(done), '| pointed', d.filter(x => pts(x) !== null).length + '/' + d.length, 'tickets,', done.filter(x => pts(x) !== null).length + '/' + done.length, 'closed');
console.log('unpointed closed:', done.filter(x => pts(x) === null).map(x => x.key).join(' ') || 'none');
