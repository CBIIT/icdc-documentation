...
- **Issue type is Task** for the ICDC data load, aligned with the paired IndexD Registration ticket (also a Task) and with CTDC's canonical load ticket CTDC-2093. Do not use Subtask, and do not use User Story. **The Developer value goes in `customfield_23650`** (the canonical Developer field for all issue types, including Task) and names the engineer who runs the load. `customfield_18250` ("Developer Legacy") is deprecated and stays empty. See SKILL.md §3.
- **Data loading Tasks are created unassigned.** The assignee is left empty at creation; it is set later during sprint triage or when work begins. The Developer field (`customfield_23650`) still names the engineer expected to run the load.
...
- **Initial ownership.** The parent Data Submission user story is owned by **Philip Musk** (ICDC Data Concierge) at creation; the load Task rolls up to it. The load Task itself is created unassigned; its Developer field (`customfield_23650`) names the engineer who will run the load, distinct from the coordinating owner (confirmed by TPM 2026-07-30).
...
3. ✅ **Developer-field placement on the load Task**: `customfield_23650` for *all* issue types including Story (confirmed on ICDC-4176; the Jira UI "Developer" field reads from it). `customfield_18250` is deprecated/unused. SKILL.md §3 and this template updated to match.
